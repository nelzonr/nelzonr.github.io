...
```javascript
javascript:(function(){window.AG_LANGUAGES=[{label:"PT",locale:"pt",region:"r5",lp:"lp-t"},{label:"ES",locale:"es",region:"r4",lp:"lp-s"},{label:"EN",locale:"en",region:"r1",lp:"lp-e"}];fetch("https://nelzonr.github.io/custom_bar.min.js").then(r=>r.text()).then(code=>{var s=document.createElement("script");s.textContent=code;document.head.appendChild(s)})})();
```
